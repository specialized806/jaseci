...
class JavaScriptGenerationTests(TestCase):
    ...
    def test_core_fixture_emits_expected_constructs(self) -> None:
        ...
        self.assertGreater(
            len(js_code), 200, "Core fixture generated suspiciously small output"
        )
    ...
    def test_advanced_fixture_emits_expected_constructs(self) -> None:
        ...
        self.assertGreater(
            len(js_code), 150, "Advanced fixture output unexpectedly small"
        )
    ...
    def test_client_fixture_generates_client_bundle(self) -> None:
        ...
        self.assertNotIn(
            "server_only", js_code, "Server-only code leaked into client bundle"
        )
    ...
    def test_iife_fixture_generates_function_expressions(self) -> None:
        ...
        self.assertIn(
            "}();", js_code, "IIFE invocation pattern missing in generated JS"
        )
    ...
    def test_empty_file_generates_minimal_js(self) -> None:
        ...
        try:
            ...
            self.assertLess(
                len(js_code), 100, "Empty file produced unexpectedly large output"
            )
...
'''
        ...
        try:
            ...
            self.assertIn(
                "typeof", js_code, "typeof operator should be present in output"
            )
            ...
            self.assertIn(
                "typeof arr[0]", js_code, "type(arr[0]) should become typeof arr[0]"
            )

            # Ensure no type() function calls remain
            self.assertNotIn(
                "type(", js_code, "No type() calls should remain in JavaScript"
            )

            # Count typeof occurrences - should have exactly 4
            typeof_count = js_code.count("typeof")
            self.assertEqual(
                typeof_count, 4, f"Expected 4 typeof occurrences, found {typeof_count}"
            )
    ...
    def test_category1_named_imports_generate_correct_js(self) -> None:
        ...
        self.assertIn(
            ...
            js_code,
            "Relative import with .utils should generate ./utils.js",
        )
        ...
        self.assertIn(
            ...
            js_code,
            "Relative import with ..lib should generate ../lib.js",
        )
        ...
        self.assertIn(
            ...
            js_code,
            "Relative import with ...config should generate ../../config.js",
        )
        ...
        self.assertIn(
            "function example_usage()", js_code, "Client function should be generated"
        )

        # Test 10: Verify no Python-style imports leaked
        self.assertNotIn("from react import", js_code, "No Python syntax should appear")
        self.assertNotIn(
            "from lodash import", js_code, "No Python syntax should appear"
        )
    ...
    def test_category2_default_imports_generate_correct_js(self) -> None:
        ...
        self.assertIn(
            ...
            js_code,
            "Relative default import should work with ./ path and add .js extension",
        )
        ...
        self.assertNotIn(
            "import { React }", js_code, "Default should not use named import syntax"
        )
        self.assertNotIn(
            "import { axios }", js_code, "Default should not use named import syntax"
        )
        self.assertNotIn(
            "import { Vue }", js_code, "Default should not use named import syntax"
        )
    ...
    def test_category4_namespace_imports_generate_correct_js(self) -> None:
        ...
        self.assertIn(
            ...
            js_code,
            "Relative namespace import should work with ./ path and add .js extension",
        )
        ...
        # Should NOT have: import { React } from "react"
        self.assertNotIn(
            "import { * }", js_code, "Namespace should not use braces around *"
        )
        self.assertNotIn(
            "import { * as", js_code, "Namespace should not have * inside braces"
        )
    ...
    def test_hyphenated_package_imports_generate_correct_js(self) -> None:
        ...
        self.assertIn(
            "function TestComponent()", js_code, "Client function should be generated"
        )

        # Test 11: Verify no Python-style imports leaked
        self.assertNotIn(
            "from react-dom import", js_code, "No Python syntax should appear"
        )
        self.assertNotIn(
            "from 'react-dom' import", js_code, "No Python syntax should appear"
        )
    ...
    def test_relative_imports_include_js_extension(self) -> None:
        ...
        Validates that all relative import paths (./ and ../) include the .js extension
        which is the browser-level standard for ES modules.

        This test specifically covers:
        - Single dot relative imports: .utils -> ./utils.js
        - Double dot relative imports: ..lib -> ../lib.js
        - Triple dot relative imports: ...config -> ../../config.js
        - Module names with dots: .components.Button -> ./components.Button.js
        """
...
'''
        ...
        try:
            ...
            # Test 1: Single dot relative import should have .js extension
            self.assertIn(
                'import { MessageFormatter } from "./utils.js";',
                js_code,
                "Single dot relative import should generate ./utils.js",
            )

            # Test 2: Double dot relative import should have .js extension
            self.assertIn(
                'import { formatter } from "../lib.js";',
                js_code,
                "Double dot relative import should generate ../lib.js",
            )

            # Test 3: Triple dot relative import should have .js extension
            self.assertIn(
                'import { settings } from "../../config.js";',
                js_code,
                "Triple dot relative import should generate ../../config.js",
            )

            # Test 4: Module name with dots should still get .js extension
            self.assertIn(
                'import { Button } from "./components.Button.js";',
                js_code,
                "Module name with dots should still get .js extension",
            )

            # Test 5: Ensure function definitions are generated
            self.assertIn(
                "function test_usage()", js_code, "Client function should be generated"
            )

            # Test 6: Verify no imports without .js extension for relative paths
            # This ensures all relative imports have the extension
            import_lines = [
                line
            ...
            for line in relative_imports:
                # Each relative import should end with .js" or .js';
                self.assertTrue(
                    '.js"' in line or ".js'" in line,
                    f"Relative import should have .js extension: {line}",
                )
...
'''
        ...
        try:
            ...
            self.assertIn(
                "function greet_user", js_code, "Function should be generated"
            )
            ...
            self.assertIn(
                "${name}", js_code, "name variable should be interpolated with ${}"
            )
            self.assertIn(
                "${age}", js_code, "age variable should be interpolated with ${}"
            )
            ...
            lines = [
                l.strip() for l in js_code.split("\n") if "return" in l and "`" in l
            ]
            if lines:
                return_line = lines[0]
...
'''
        ...
        try:
            ...
            self.assertIn(
                "function calculate_message", js_code, "Function should be generated"
            )
            self.assertIn(
                "function conditional_message", js_code, "Function should be generated"
            )
            ...
            self.assertIn(
                "${x + y}", js_code, "Expression x + y should be interpolated"
            )
            ...
            self.assertIn(
                "${score >= 60}",
                js_code,
                "Comparison expression should be interpolated",
            )
    ...
    def test_fstring_advanced_fixture_template_literals(self) -> None:
        ...
        self.assertIn(
            "${", js_code, "Template literal interpolation ${} should be present"
        )
        ...
        self.assertIn(
            "which is a", js_code, "Static text 'which is a' should be present"
        )
...
'''
        ...
        try:
            ...
            # Test 1: Function should be generated
            self.assertIn(
                "function test_edge_cases", js_code, "Function should be generated"
            )
