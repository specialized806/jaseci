...
class SemDefMatchPassTests(TestCase):
    ...
    def test_sem_def_match(self) -> None:
        ...
        self.assertEqual(
            mod.lookup("E").decl.name_of.semstr, "An enum representing some values."
        )  # type: ignore
        self.assertEqual(
        ...
        self.assertEqual(
            mod.lookup("Person").decl.name_of.semstr, "A class representing a person."
        )  # type: ignore

        person_scope = mod.lookup("Person").symbol_table  # type: ignore
        self.assertEqual(
            person_scope.lookup("name").decl.name_of.semstr, "The name of the person."
        )  # type: ignore
        self.assertEqual(
        ...
        self.assertEqual(
            sym_calc_age.decl.name_of.semstr, "Calculate the age of the person."
        )  # type: ignore
