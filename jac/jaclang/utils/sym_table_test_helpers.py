...
class SymTableTestMixin(TestCase):
    ...
    def assert_symbol_exists(self, sym_table: UniScopeNode, symbol_name: str, symbol_type: str = None) -> None:
        """Assert that a symbol exists in the symbol table."""
        symbol = look_down(sym_table,symbol_name)
        self.assertIsNotNone(symbol, f"Symbol '{symbol_name}' not found in symbol table")
        if symbol_type:
            self.assertIn(symbol_type, str(symbol), f"Symbol '{symbol_name}' is not of type '{symbol_type}'")
        return symbol
    ...
    def assert_symbol_uses_at(self, symbol:Symbol, expected_uses: list[tuple[int, int]]) -> None:
        ...
        for line, col in expected_uses:
            ...
            self.assertIn(expected_use, symbol_uses_str,
                         f"Symbol use not found at {expected_use}. Got: {symbol_uses_str}")

    def assert_symbol_complete(self, sym_table, symbol_name: str, symbol_type: str,
                              decl: tuple[int, int], defns: list[tuple[int, int]] = None,
    ...
    def assert_sub_table_exists(self, sym_table:UniScopeNode, table_name: str,tab_type:str) -> None:
        ...
        matching_types = [type_name for type_name in type_names if tab_type in str(type_name)]
        self.assertTrue(len(matching_tables) > 0, 
                       f"Sub-table '{table_name}' not found. Available: {table_names}")
        self.assertTrue(len(matching_types) > 0,
