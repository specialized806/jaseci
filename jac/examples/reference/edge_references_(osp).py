...
class EdgeRefWalker(Walker):
    ...
    def demonstrate(self, here: Root) -> None:
        ...
        print('From root:')
        out_from_root = refs(Path(here)._out())
    ...
    def show_refs(self, here: Person) -> None:
        if here.name == 'Alice':
            ...
            outgoing = refs(Path(here)._out())
            print(f'Outgoing [-->]: {len(outgoing)} nodes')
            for n in outgoing:
                print(f'  - {n.name}')
            incoming = refs(Path(here)._in())
            print(f'\\nIncoming [<--]: {len(incoming)} nodes')
            both = refs(Path(here)._any())
            print(f'\\nBidirectional [<-->]: {len(both)} nodes')
            print('\n=== 3. Typed Edge References ===\n')
            friends = refs(Path(here)._out(edge=lambda i: isinstance(i, Friend)))
            print(f'Friend edges [->:Friend:->]: {len(friends)} nodes')
            for n in friends:
                print(f'  - {n.name}')
            colleagues = refs(Path(here)._out(edge=lambda i: isinstance(i, Colleague)))
            print(f'Colleague edges [->:Colleague:->]: {len(colleagues)} nodes')
            for n in colleagues:
                print(f'  - {n.name}')
            print('\n=== 4. Filtered Edge References ===\n')
            old_friends = refs(Path(here)._out(edge=lambda i: isinstance(i, Friend) and i.since < 2018))
            print(f'Friends since before 2018: {len(old_friends)} nodes')
            experienced_colleagues = refs(Path(here)._out(edge=lambda i: isinstance(i, Colleague) and i.years > 2))
            print(f'Colleagues with years > 2: {len(experienced_colleagues)} nodes')
            specific = refs(Path(here)._out(edge=lambda i: isinstance(i, Colleague) and i.years >= 1 and (i.years <= 5)))
            print(f'Colleagues with 1-5 years: {len(specific)} nodes')
            print('\n=== 5. Edge and Node Keywords ===\n')
            edge_objs = refs(Path(here)._out().edge())
            print(f'[edge -->]: Retrieved {len(edge_objs)} edge objects')
            node_objs = refs(Path(here)._out())
            print(f'[node -->]: Retrieved {len(node_objs)} node objects')
            print('\n=== 6. Chained Edge References ===\n')
            david = Person(name='David')
            bob_list = refs(Path(here)._out(edge=lambda i: isinstance(i, Friend)))
            if bob_list:
                connect(left=bob_list[0], right=david, edge=Friend(since=2018))
            two_hop = refs(Path(here)._out(edge=lambda i: isinstance(i, Friend))._out(edge=lambda i: isinstance(i, Friend)))
            print(f'[here ->:Friend:-> ->:Friend:->]: {len(two_hop)} nodes (2 hops via Friend)')
            mixed = refs(Path(here)._out(edge=lambda i: isinstance(i, Friend))._out(edge=lambda i: isinstance(i, Colleague)))
            ...
            targets = refs(Path(here)._out())
            print(f'Assignment: targets = [-->] → {len(targets)} nodes')
            if refs(Path(here)._out()):
                print('Conditional: if [-->] → edges exist!')
            print('For loop:')
            for person in refs(Path(here)._out()):
