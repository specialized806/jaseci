...
class ComprehensionDemo(Walker):
    ...
    def demo_basic(self, here: Root) -> None:
        ...
        result = filter_on(items=filter_on(items=filter_on(items=data, func=lambda i: i.x > 2), func=lambda i: i.y < 15), func=lambda i: i.z >= 6)
    ...
    def demo_edge_filters(self, here: Employee) -> None:
        ...
        all_reports = refs(Path(here)._out())
        ...
        reports_via_edge = refs(Path(here)._out(edge=lambda i: isinstance(i, ReportsTo)))
