...
class VisitWithElse(Walker):
    ...
    def start(self, here: Root) -> None:
        ...
        if not visit(self, refs(Path(here)._out().visit())):
...
class TypedVisit(Walker):
    ...
    def start(self, here: Person) -> None:
        ...
        visit(self, refs(Path(here)._out(edge=lambda i: isinstance(i, Friend)).visit()))
...
class FilteredVisit(Walker):
    ...
    def start(self, here: Person) -> None:
        ...
        visit(self, refs(Path(here)._out(edge=lambda i: isinstance(i, Colleague) and i.strength > 5).visit()))
