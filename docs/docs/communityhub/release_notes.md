...
- **Formatted String Literals (f-strings)**: Added comprehensive support for Python-style formatted string literals in Jac with full feature parity including:
  - Basic f-string syntax with `f"Hello {name}"` and `F'Hello {name}'`
  - Expression evaluation and numeric formatting (hex, binary, decimal precision, field width)
  - String conversion flags (`!r`, `!s`, `!a`) for repr(), str(), and ascii() representations
  - Nested f-strings for complex string composition
  - Multiline f-strings using triple quotes (`f"""` and `f'''`)
  - Raw f-strings (`rf"` and `fr"`) for literal backslashes, file paths, and regex patterns
  - Proper escaping of curly braces with `{{` and `}}`
  - Complete compatibility with existing Python f-string syntax and semantics
