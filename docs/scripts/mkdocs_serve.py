...
class DebouncedRebuildHandler(FileSystemEventHandler):
    ...
    def __init__(self, root_dir: str, debounce_seconds: int = 10, ignore_paths=None) -> None:
    ...
    def _should_ignore(self, path: str) -> bool:
        ...
        return any(os.path.commonpath([os.path.abspath(path), os.path.abspath(ignore)]) == os.path.abspath(ignore)
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if not event.is_directory and "site" not in event.src_path and not self._should_ignore(event.src_path):
...
def serve_with_watch() -> None:
    ...
    # Set up file watcher
    event_handler = DebouncedRebuildHandler(root_dir=root_dir, debounce_seconds=20, ignore_paths=ignore_paths)
    observer = Observer()
