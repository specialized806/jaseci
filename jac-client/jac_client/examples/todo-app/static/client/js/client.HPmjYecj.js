(function() {
  ...
  function __jacJsx(tag, props, children) {
    ...
    if (reactChildren.length > 0) {
      ...
      return reactExports.createElement.apply(React, args);
    } else {
  ...
  async function __jacSpawn(walker, fields, nd2) {
    let token = __getLocalStorage("jac_token");
    let response = await fetch(`/walker/${walker}`, { "method": "POST", "headers": { "Content-Type": "application/json", "Authorization": token ? `Bearer ${token}` : "" }, "body": JSON.stringify({ "nd": nd2 ? nd2 : "root", ...fields }) });
    if (!response.ok) {
  ...
  async function toggleTodo(id2) {
    await __jacSpawn("toggle_todo", {}, id2);
    let s = todoState();
    let updated = [];
